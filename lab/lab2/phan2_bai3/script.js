...
for (let i = 1; i <= 31; i++) {
  ...
  option.value = i;
  option.text = i;
  document.querySelector(".date-picker__day").appendChild(option);
}
...
for (let i = 0; i < months.length; i++) {
  ...
  document.querySelector(".date-picker__month").appendChild(option);
}
...
for (let i = currentYear; i >= currentYear - 100; i--) {
  ...
  option.value = i;
  option.text = i;
  document.querySelector(".date-picker__year").appendChild(option);
}
...
const date=document.getElementById("Birthday").querySelectorAll("select");
...
const otherRadio = document.querySelector('input[name="gender"][value="other"]');
const country=document.getElementById("custom-select-input");
...
function validate(){
  ...
  const ml=mail.value.trim();
  const pw=password.value;
  const ab=about.value.trim();
  const regName=/^[a-zA-Z]+$/;
  const regMail=/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
  if(!regName.test(fname)){
    alert(fname+" is a invalid first name! Please enter letters only");
     firstName.focus();
     return false;
  } else if(fname.length <2 || fname.length>30){
    alert('First name is too short or too long!');
    return false;
  }
  if(!regName.test(lname)){
    ...
    alert('Last name is too short or too long!');
    lastName.focus();
    return false;
  }
  if(!regMail.test(ml)){
  ...
  if(pw.length<2||pw.length>30){
    alert("Invalid password!");
    password.focus();
    return false;
  }
  if(ab.length>10000){
    alert("Say about u a litter, man!");
    about.focus();
    return false;
  }
  if(date[1].selectedIndex==0||date[0].selectedIndex==0||date[2].selectedIndex==0){
    alert("Please, finish your birthday!");
    return false;
  } 
  if(!maleRadio.checked && !femaleRadio.checked && !otherRadio.checked){
    alert("Please, choose a sexual orientation!");
    return false;
  }
  console.log(country.value);
  if(country.value==""){
    alert("Tell me where u come from!");
    return false;
  }
  if(ab.length==0){
    alert("Tell me about u");
    about.focus();
    return false;
  }
   alert("Successful!");
